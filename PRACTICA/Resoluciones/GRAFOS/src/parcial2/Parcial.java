...
public class Parcial {
		...
		if(!sitios.isEmpty()) {
			...
			if (origen!=null) {
				...
				if(tiempoOrigen<=tiempo) {
					cant=this.dfs(origen,sitios,tiempo-tiempoOrigen,1,
							Integer.MIN_VALUE,new boolean[sitios.getSize()]);
				}
			}
		}
		return cant;
	...
	private int dfs(Vertex<Recinto> origen,Graph<Recinto> grafo,int tiempo,int cant
			,int max,boolean[]marcas) {
		...
		for (Edge<Recinto> ady: grafo.getEdges(origen)) {
			...
			if (!marcas[j] && peso<=tiempo) {
				cant=this.dfs(destino, grafo, tiempo-peso, cant+1, max, marcas);
			}
		}
    ...
    public static void main(String args[]) {
        ...
        System.out.println(p.resolver(grafo, 60));
