...
public class Parcial {
	...
	private boolean dfs(Graph<String>ciudades,Vertex<String> v1,Vertex<String>v2,List<String>lis,boolean[]marcas,int montoMaximo) {
		...
		if (v1==v2) {
			...
			for (Edge<String> ady: ciudades.getEdges(v1)) {
				Vertex aux=ady.getTarget();
				int peso=ady.getWeight();
				if (!marcas[aux.getPosition()] && peso<=montoMaximo) {
					encontre=this.dfs(ciudades, aux, v2, lis, marcas, montoMaximo-peso);
    ...
    public static void main(String args[]) {
        ...
        System.out.println(p.caminoConPresupuesto(grafo, "Lincoln", "Berisso", 1000));
    }
}
