...
public class Parcial {
	...
	private boolean dfs(Graph<String> ciudades,Vertex<String> origen,Vertex<String>destino
			,List<String>lis,boolean[]marcas,int montoMinimo,int actual) {
		boolean encontre=false;
		marcas[origen.getPosition()]=true;
		lis.add(origen.getData());
		if(origen==destino && actual>montoMinimo) { //con solo cambiar > por un < buscaria el camino menos costoso
			return true;
		}else {
			for (Edge<String> ady:ciudades.getEdges(origen)) {  //RECORRO LOS ADYACENTES DEL VERTICE ACTUAL
				Vertex<String> aux=ady.getTarget();
				int peso=ady.getWeight();
				if(!marcas[aux.getPosition()]&& !encontre) {
	...
	public static void main(String args[]) {
		...
		Vertex<String> v2=grafo.createVertex("Chascomus");
		Vertex<String> v3=grafo.createVertex("Cañuelas");
		Vertex<String> v4=grafo.createVertex("Dolores");
		Vertex<String> v5=grafo.createVertex("Veronica");
