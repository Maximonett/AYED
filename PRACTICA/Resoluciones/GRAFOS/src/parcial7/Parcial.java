...
public class Parcial {
	public List<String> recorrido(Graph<String> grafo,int cantLocalidades
			,int cantNafta,List<String> localidadesExceptuadas){
		...
		if(!grafo.isEmpty()) {
			...
			if(origen!=null) {
				...
				this.dfs(grafo,origen,cantLocalidades,cantNafta,1,camino,marcas);
			}
		}
		return camino;
	}
	...
	private boolean dfs(Graph<String>grafo, Vertex<String>origen,int cantLocalidades
			, int cantNafta, int cant,List<String>camino,boolean[]marcas){
		...
		if (cantLocalidades==cant) {
			...
			for(Edge<String> ady:grafo.getEdges(origen)) {
				...
				if(!marcas[aux.getPosition()] && peso<=cantNafta) {
					encontre=this.dfs(grafo, aux, cantLocalidades, cantNafta-peso
							,cant+1,camino, marcas);
				}
			}
		}
	...
	public static void main(String args[]) {
		...
		Vertex<String> v2=grafo.createVertex("Tunuyan");
		...
		List<String> localidadesExceptuadas=new LinkedList<>();
