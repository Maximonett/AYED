...
public class Parcial {
	...
	private Vertex<Recinto> buscar(Graph<Recinto> sitios) {
		...
		for (Vertex<Recinto> aux : sitios.getVertices()) {
	...
	/*sitios: el grafo que contiene los recintos.

		origen: el nodo actual desde el que partimos.

		tiempo: tiempo restante disponible para visitar.

		cant: cantidad de sitios visitados hasta ahora.

		max: cantidad total de sitios que queremos visitar.

		marcas: vector booleano que indica qué nodos ya visitamos (para evitar ciclos).*/
	
	private boolean dfs(Graph<Recinto> sitios,Vertex<Recinto> origen
			,int tiempo,int cant,int max,boolean[]marcas) {
		boolean encontre=false;
		marcas[origen.getPosition()]=true;
		if (cant==max) {
			return true;
		}else {
			for (Edge<Recinto> ady:sitios.getEdges(origen)) {
				if (encontre) break;
	...
	public static void main(String[] args) {
        ...
        System.out.println(p.resolver(grafo, 98));
