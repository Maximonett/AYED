...
public class Parcial {
	public List<String> estadios(Graph<Estadio> mapaEstadios
			,String estadioOrigen,int cantKm){
		...
		if (!mapaEstadios.isEmpty()) {
			...
			if(origen!=null) {
				this.dfs(mapaEstadios,origen,camMax,new LinkedList<String>()
						,new boolean[mapaEstadios.getSize()],cantKm);
			}
		}
		return camMax;
	}
	...
	private Vertex<Estadio> buscar(Graph<Estadio>mapaEstadios,String estadioOrigen){
		for (Vertex<Estadio> aux:mapaEstadios.getVertices()) {
	...
	private void dfs(Graph<Estadio> mapaEstadio,Vertex<Estadio> origen,List<String> camMax
			,List<String>camAct,boolean[]marcas,int cantKm) {
		marcas[origen.getPosition()]=true;
		camAct.add(origen.getData().getNombre());
		for (Edge<Estadio> ady: mapaEstadio.getEdges(origen)) {
			int peso=ady.getWeight();
			Vertex destino=ady.getTarget(); //PIDO EL VERTICE DESTINO
			if (!marcas[destino.getPosition()] && peso<=cantKm) {
				this.dfs(mapaEstadio, destino, camMax, camAct, marcas, cantKm-peso);
			}
		}
    ...
    public static void main(String args[]) {
        ...
        System.out.println(p.estadios(grafo, "AI BAYT STADIUM", 1000));
    }
}
