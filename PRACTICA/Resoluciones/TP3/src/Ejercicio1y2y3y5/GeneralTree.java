...
public class GeneralTree<T> {
	...
	public List<Integer> numerosImparesMayoresQuePreOrden(Integer n){
		...
		if (!this.isEmpty()) numerosImparesMayoresQuePreOrden(n,l);
		return l;
	...
	private List<Integer> numerosImparesMayoresQuePostOrden(Integer n){
		List<Integer> l=new LinkedList<>();
	...
	public int altura() {
		return (!this.isEmpty()) ? alturaHelper(): -1; //Si el arbol es vacio devuelve -1, un arbol vacio se considera de altura -1
	}
	
	private int alturaHelper() {
		if (this.isLeaf()) return 0; //caso base si es el nodo actual es un a hoja no tiene hijos entonces es de altura 0
		else {
			int alturaMax=-1;
			List<GeneralTree<T>> children=this.getChildren();
			for (GeneralTree<T> child:children) {
	...
	private int nivelHelper(T dato) {
		...
		cola.enqueue(this);
		while(!cola.isEmpty()) {
			act=cola.size(); //tamaño de la cola
			for (int i=0; i<act;++i) {
				aux=cola.dequeue();
	...
	public boolean esAncestro(T a, T b): devuelve true si el valor “a” es ancestro del valor “b”. */
	    ...
	    public static void main(String[] args) {
	       ...
	       GeneralTree<Integer> gen=new GeneralTree<>();
	        
	        System.out.println("PREORDEN:" + gen.numerosImparesMayoresQuePreOrden(0));
	        System.out.println("INORDEN:" + gen.numerosImparesMayoresQueInOrden(0));
	        System.out.println("POSTORDEN:" + gen.numerosImparesMayoresQuePostOrden(0));
	        System.out.println("POR NIVELES:" + gen.numerosImparesMayoresQuePorNiveles(a, 0)); 
	    }
